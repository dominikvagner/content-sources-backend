...
func (r *pulpDaoImpl) UpdateDomain(ctx context.Context, pulpHref string, domainPartial zest.PatchedDomain) error {
	ctx, client := getZestClient(ctx)
	_, resp, err := client.DomainsAPI.DomainsPartialUpdate(ctx, pulpHref).PatchedDomain(domainPartial).Execute()
	if resp != nil && resp.Body != nil {
		defer resp.Body.Close()
	}
	if err != nil {
...
func (r *pulpDaoImpl) CreateDomain(ctx context.Context, name string) (string, error) {
	...
	domain.Description = *zest.NewNullableString(utils.Ptr("content-sources"))
